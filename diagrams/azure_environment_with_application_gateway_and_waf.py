...
from diagrams.azure.network import LoadBalancers, NetworkSecurityGroupsClassic, VirtualNetworks, Subnets, ApplicationGateway
...
with Diagram("Azure Environment with Application Gateway and WAF", show=False):
    ...
    with Cluster("Azure Cloud"):
        ...
        ssl_certs = KeyVaults("Key Vault\nSSL Certificates")
    
    # Connect components for traffic flow
    internet >> Edge(label="HTTPS (TCP 443)") >> app_gateway
    app_gateway >> Edge(label="WAF Filtered Traffic") >> lb
    lb >> Edge(label="Load Balanced Traffic") >> backend_nsg
    backend_nsg >> Edge() >> vm01
    backend_nsg >> Edge() >> vm02
    
    # SSL Certificate connections
    ssl_certs >> Edge(label="TLS Certificates") >> app_gateway
